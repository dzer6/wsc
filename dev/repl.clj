...
(def aliases
  {'config   'base.configuration.api
   'query    'wiki-stem-corpus.db.query
   'postgres 'wiki-stem-corpus.db.postgres
   'io       'clojure.java.io
...
(def db-query-wrappers-declaration-path "src/main/clj/wiki_stem_corpus/db/query.clj")

(def db-query-wrappers-declaration-template "(ns wiki-stem-corpus.db.query)
...
(defn keyword->state [kw]
  (get {:web        'wiki-stem-corpus.server/*runner*
        :config     'wiki-stem-corpus.config/*loader*
        :postgres   'wiki-stem-corpus.db.postgres/*database*
        :migrations 'wiki-stem-corpus.migrations/*runner*}
       kw))
...
(defn run-tests
  ([]
   (run-tests #"wiki-stem-corpus.*?\-test"))
  ([regular-expression]
   (mount/stop)
