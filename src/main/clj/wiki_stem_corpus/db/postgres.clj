(ns wiki-stem-corpus.db.postgres
...
(defn start! []
  (when (and (nil? @db) (config/feature-on? @app-config/holder :database :postgres))
    ...
    (postgres-component/generate-query-wrappers 'wiki-stem-corpus.db.query
                                                queries-file-path)
...
(defn stop! []
  (when (some? @db)
    ...
    (reset! db nil)
    (postgres-component/remove-generated-query-wrappers 'wiki-stem-corpus.db.query
                                                        queries-file-path)))
